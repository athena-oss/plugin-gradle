...
#
# Set project directory inside the container
athena.docker.add_env "PROJECT_DIR_IN_CONTAINER" "/mnt/${project_dir}"

# In Mac OS we run gradle against a rsynced directory to avoid the bad performance
# of docker for mac OXFS
athena.debug "Using '$project_dir' as /opt/project inside the container.."
...
if athena.os.is_mac && ! athena.argument.argument_exists_and_remove "--skip-sync"; then
	...
	athena.docker.volume_exists_or_create "athena_cache_gradle_mounts"
	athena.docker.add_option -v "athena_cache_gradle_mounts:/mnt"
else
	athena.docker.mount_dir "$project_dir" "/mnt/${project_dir}"
fi
